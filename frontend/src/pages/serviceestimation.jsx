...
const ServiceEstimation = () => {
  ...
  const [serviceInvoice, setServiceInvoice] = useState({ invoice_date: new Date().toISOString().slice(0, 10) });
  ...
  const resetForm = () => {
    ...
    setDescInput("");
    setServiceInvoice({ invoice_date: new Date().toISOString().slice(0, 10) });
    setExtra(emptyExtra());
    setEditId(null);
  };
    ...
    <div className="w-full">
      ...
      <div className="invoice-heading-tab flex gap-4 justify-between items-center flex-wrap">
        ...
        <div className="flex gap-3 flex-wrap">
          ...
          <div className="flex items-center gap-2 mt-2">
            <button onClick={() => { if (invoiceRef.current) { html2pdf().from(invoiceRef.current).set({ margin: 10, filename: `Service_Estimation_${viewId}.pdf`, image: { type: "jpeg", quality: 0.98 }, html2canvas: { scale: 2 }, jsPDF: { unit: "mm", format: "a4", orientation: "portrait" } }).save(); } }} title="Download PDF" className="w-10 h-10 bg-white border rounded-lg shadow-sm flex justify-center items-center hover:bg-gray-50 transition"><Download size={20} /></button>
      ...
      {viewId && (
        <div key={viewId} ref={invoiceRef} className={`invoicewrapper w-full mt-6 bg-white shadow-xl p-6 relative overflow-y-auto ${showinvoice ? "See" : ""}`}>
          ...
          <Invoice serviceEstimationId={viewId} />
        </div>
      )}
    </div>
  );
